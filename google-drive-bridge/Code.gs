...
function updateInterviewAppointment048_(payload) {
  ...
  while (files.hasNext()) {
    const entryFile = files.next();
    if (entryFile.getMimeType() === MimeType.GOOGLE_SHEETS && entryFile.getName().indexOf('Интервью на продуктивность — ') === 0) matches.push(entryFile);
  }
  ...
  const appointmentFile = matches[0];
  const book = SpreadsheetApp.openById(appointmentFile.getId());
  const sheet = book.getSheetByName('Начало');
