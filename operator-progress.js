...
(() => {
  let evidence={};
  const names={new:'Анкета 1 / выбор Zoom',experienced_not_target:'Опыт от года — без приглашения',interview_booked:'Записан на первичный Zoom',interviewed:'Первичный Zoom пройден',questionnaire:'Анкета 2 / Тест 1',test_1_completed:'Тест 1 заполнен — ожидает решения',test_1_passed:'Тест 1 проверен — допущен',productivity_invited:'Приглашён на продуктивность',productivity_booked:'Записан на продуктивность',finalist:'Финал отбора',selection_closed:'Не прошёл — ожидается ответ',rejected:'Отбор завершён',cancelled:'Отказался сам',academy_contact:'Контакт Академии',training:'Обучение',internship:'Стажировка',hired:'Принят'};
  ...
  function view(){
    ...
    let next=closed?'Решение / дальнейший этап: '+label(c.status):active||legacy?'Ожидаем встречу по выбранному времени':errors?'Журнал приглашений загружен не полностью — обновите карточку':invitation||invites.length?'Приглашение есть в истории. Проверьте дату и выбор времени ниже':testDetails?.submitted_at?'Тест 1 заполнен. Подтверждения приглашения на продуктивность не найдено: требуется решение и приглашение оператора':!questionnaireTwo?.submitted_at&&['interviewed','questionnaire'].includes(c.status)?'Ожидаем Анкету 2':questionnaireTwo?.submitted_at?'Ожидаем Тест 1 по инструкциям группы':c.status==='interview_booked'?'Ожидаем первичный Zoom и кодовое слово после встречи':'Ожидаем продолжение кандидатом в боте';
