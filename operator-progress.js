...
(() => {
  ...
  const render=renderDetail;renderDetail=()=>{
    ...
    select.onchange=async()=>{const value=select.value;select.disabled=true;feedback.textContent='Сохраняем…';try{await api('PATCH',{candidateId,status:value});saved=value;const row=all.find(c=>c.id===candidateId);if(row)row.status=value;if(details?.id===candidateId){details.status=value;progress.innerHTML=view();renderList();}feedback.textContent='Сохранено. Сообщения не отправлялись.';}catch(error){select.value=saved;feedback.textContent='Не сохранено: '+error.message;}finally{select.disabled=false;}};
