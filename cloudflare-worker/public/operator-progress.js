...
(() => {
  let evidence={};
  if(!statuses.some(s=>s[0]==='productivity_failed'))statuses.push(['productivity_failed','Продуктивность не пройдена']);
  if(!statuses.some(s=>s[0]==='productivity_passed'))statuses.push(['productivity_passed','Продуктивность пройдена — расшифровать Тест 1']);
  statuses.push(['test_1_incomplete_removed','Не заполнил. Исключён']);
  const stageOrder=['new','experienced_not_target','interview_booked','interviewed','questionnaire','test_1_incomplete_removed','test_1_completed','productivity_invited','productivity_booked','productivity_passed','productivity_failed','test_1_passed','finalist','selection_closed','rejected','cancelled','academy_contact','training','internship','hired'];
  ...
  const oldApply=applyCandidateData;applyCandidateData=d=>{oldApply(d);evidence=d.progress||{errors:['not_loaded']};if(details)details.primary_zoom_clicked_at=evidence.primaryEntry?.[0]?.clicked_at||null;};
  ...
  function view(){
    ...
    (p.declines||[]).forEach(e=>rows.push(['Отказ / уведомление коллегам',date(e.created_at)+' · '+(e.notified_at?'Бриф отправлен '+date(e.notified_at):e.error?'Требует проверки: '+e.error:'Бриф ожидает отправки')]));
    ...
    const botEvent=(p.botEvents||[])[0];
    const closed=['test_1_incomplete_removed','rejected','cancelled','selection_closed','academy_contact','finalist','training','internship','hired'].includes(c.status);
    let next=c.status==='productivity_failed'?'Продуктивность не пройдена. Дальнейшее тестирование не продолжаем. Сообщение кандидату — отдельным действием оператора':c.status==='productivity_passed'?'Интервью на продуктивность пройдено. Теперь расшифруйте Тест 1':c.status==='test_1_passed'?'Тест 1 расшифрован. Ожидается решение по дальнейшему этапу':closed?'Решение / дальнейший этап: '+label(c.status):active||legacy?'Ожидаем встречу по выбранному времени':errors?'Журнал приглашений загружен не полностью — обновите карточку':invitation||invites.length?'Приглашение есть в истории. Проверьте дату и выбор времени ниже':testDetails?.submitted_at?'Тест 1 заполнен. Подтверждения приглашения на продуктивность не найдено: следующий шаг — интервью на продуктивность, без предварительной расшифровки теста':!questionnaireTwo?.submitted_at&&['interviewed','questionnaire'].includes(c.status)?'Ожидаем Анкету 2':questionnaireTwo?.submitted_at?'Ожидаем Тест 1 по инструкциям группы':c.status==='interview_booked'?'Ожидаем первичный Zoom и кодовое слово после встречи':'Ожидаем продолжение кандидатом в боте';
    rows.push(['Вы такой человек?',c.garcia_confirmed===true||c.test_answer==='Послание к Гарсии: да'?'☑ Да':c.test_answer==='Послание к Гарсии: нет'?'☐ Нет (сохранённый ответ)':c.test_answer==='Послание к Гарсии: не отмечено'?'☐ Галочка не отмечена':'Ответ не сохранён в этой версии анкеты']);
    rows.push(['Последнее событие бота',!botEvent?'Журнал событий появится после следующего сообщения':botEvent.state==='done'?'Обработано · '+date(botEvent.updated_at):botEvent.state==='processing'?'Выполняется · попытка '+botEvent.attempts:botEvent.state==='attention'?'Требует внимания · попытка '+botEvent.attempts+(botEvent.error?' · '+botEvent.error:''):'Состояние: '+botEvent.state]);
    rows.push(['Анкета 2',stage(questionnaireTwo)],['Тест 1',stage(testDetails)],['Первое Zoom-собеседование',c.interview_at?date(c.interview_at)+(new Date(c.interview_at)<new Date()?' · прошедшая дата':''):primary?'Есть подтверждение в переписке':'Запись не подтверждена']);
    rows.push(['Первое нажатие входа в Zoom',c.primary_zoom_clicked_at?date(c.primary_zoom_clicked_at)+' · не подтверждает присутствие':'Не зафиксировано; старые URL-кнопки нажатий не передавали']);
    (p.timers||[]).forEach(t=>rows.push([({primary_session:'Первое Zoom: напоминание / контроль неявки',minsk_review_30m:'Минск: напоминание и бриф за 30 минут',primary_entry_report:'Отчёт о нажатии Zoom'})[t.kind]||t.kind,date(t.due_at)+' · '+({pending:'Назначено',running:'Выполняется',done:'Обработка завершена',attention:'Требует проверки'})[t.state]+(t.error?' · '+t.error:'')]));
    rows.push(['Цели Академии','В приглашении после Теста 1: страница и PDF. Факт прочтения не отслеживается']);
  ...
  const render=renderDetail;renderDetail=()=>{
    ...
    const controls=document.createElement('div');controls.className='manual-stage';
    const caption=document.createElement('label');caption.htmlFor='status';caption.textContent='Этап кандидата — изменить вручную';
    const feedback=document.createElement('span');feedback.setAttribute('role','status');
    controls.append(caption,select,feedback);head.prepend(controls);
    const candidateId=details.id;let saved=details.status;
    select.onchange=async()=>{const value=select.value;select.disabled=true;feedback.textContent='Сохраняем…';try{const result=await api('PATCH',{candidateId,status:value});saved=value;const row=all.find(c=>c.id===candidateId);if(row)row.status=value;if(details?.id===candidateId){details.status=value;if(value==='cancelled'){evidence.offlineBookings=[];evidence.bookings=[];}progress.innerHTML=view();renderList();}feedback.textContent=result.warning||(value==='cancelled'?`Отказ сохранён. Освобождено мест: ${result.freed||0}. Бриф коллегам: ${result.notification==='sent'?'отправлен':'поставлен на отправку'}.`:'Сохранено. Сообщения не отправлялись.');}catch(error){select.value=saved;feedback.textContent='Не сохранено: '+error.message;}finally{select.disabled=false;}};
  ...
  .manual-stage{display:flex;flex-direction:column;gap:6px;margin-bottom:12px}.manual-stage label{font-weight:700}.manual-stage select{width:100%;min-height:42px}.manual-stage [role=status]{font-size:12px}
