...
- Callback reliability: Telegram's harmless `query is too old / query ID is invalid` response for an already delayed button acknowledgement no longer turns a completed candidate action into a failed webhook event or retry loop.
