...
export function eligibility(candidate, config, session) {
  ...
  if (['invite', 'test_passed'].includes(config.action)) {
    ...
    if (['test_1_incomplete_removed', 'finalist', 'training', 'internship', 'hired', 'rejected', 'cancelled', 'academy_contact', 'collaboration', 'inactive', 'selection_closed', 'productivity_failed', 'talent_pool'].includes(candidate.status)) return 'Этап уже завершён';
