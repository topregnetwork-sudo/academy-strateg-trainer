...
async function coordinate(key, text, keyboard, city) {
  return effect(key, () => telegram(COORD, text, { message_thread_id: reviewThread(city), ...(keyboard ? {reply_markup: keyboard} : {}) }));
}
...
export async function book(sessionId, slotId, candidateId) {
  return transaction(async tx => {
    ...
    if (!c || ['rejected','cancelled','academy_contact','selection_closed','productivity_passed','productivity_failed','talent_pool','reserve_no_response','test_1_passed','offline_testing','finalist','hired','training','internship'].includes(c.status) || (session.config.city!=='Все города'&&c.city !== session.config.city)) throw new Error('Запись для вашего этапа недоступна');
    ...
    if(old&&session.config.allowReschedule===false)throw new Error('Ваше время уже подтверждено. Для изменений свяжитесь с организатором.');
...
export async function bookingFollowup(bookingId, version) {
  ...
  if(!c||c.consent===false||['cancelled','rejected','selection_closed','academy_contact','productivity_failed','reserve_no_response','offline_testing'].includes(c.status))return {done:true};
...
export async function sendSessionSummary(sessionId,key,slotId=null) {
  ...
  const days=[...new Set(rows.map(r=>new Date(r.starts_at).toLocaleDateString('ru-RU',{timeZone:'Europe/Moscow'})))];
  const lines=[`📋 <b>${slotId?'Встреча скоро начнётся':'Ближайшие записи'} — интервью на продуктивность</b>`,`Дни: ${esc(days.join(', ')||'нет будущих записей')}`];
  for(const entries of groups.values()){
    const people=entries.filter(r=>r.first_name||r.username||r.last_name),s=entries[0];
    lines.push(`\n<b>${new Date(s.starts_at).toLocaleDateString('ru-RU',{timeZone:'Europe/Moscow',day:'2-digit',month:'2-digit'})} ${new Date(s.starts_at).toLocaleTimeString('ru-RU',{timeZone:'Europe/Moscow',hour:'2-digit',minute:'2-digit'})}: ${people.length} / ${s.capacity}</b>`);
...
export async function handleFunnelCallback(callback) {
  ...
  try{
    ...
    if(match){const session=await sessionById(Number(match[1]));if(session?.config.allowReschedule===false)throw new Error('Изменение времени для этой встречи закрыто. Свяжитесь с организатором.');await telegram(c.chat_id,'Выберите другое время. Прежнее место сохраняется до подтверждения нового.',{reply_markup:await sessionKeyboard(Number(match[1]),c.id)});await telegramApi('answerCallbackQuery',{callback_query_id:callback.id});return true;}
