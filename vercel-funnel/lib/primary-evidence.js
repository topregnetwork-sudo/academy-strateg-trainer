...
// The reminder is sent 30 minutes before the meeting, so early button
// presses must remain valid evidence for the whole reminder-to-meeting period.
export const PRIMARY_ENTRY_WINDOW_MINUTES=60;
...
export async function primaryAccess(id){
  ...
  const row=(await sql`SELECT (SELECT MIN(e.clicked_at) FROM (SELECT * FROM candidate_zoom_entries UNION ALL SELECT candidate_id,clicked_at,interview_at,slot_id FROM candidate_zoom_session_entries) e WHERE e.candidate_id=c.id AND e.clicked_at BETWEEN e.interview_at-(${PRIMARY_ENTRY_WINDOW_MINUTES} * INTERVAL '1 minute') AND e.interview_at+INTERVAL '60 minutes') AS clicked_at,
    (SELECT MIN(m.created_at) FROM messages m WHERE m.candidate_id=c.id AND m.kind='primary_zoom_link' AND m.created_at BETWEEN c.interview_at-(${PRIMARY_ENTRY_WINDOW_MINUTES} * INTERVAL '1 minute') AND c.interview_at+INTERVAL '60 minutes') AS message_clicked_at,
...
export async function reportPrimaryEntry(id){
  ...
  if(!c)return;
  const count=(await sql`SELECT count(DISTINCT candidate_id)::int AS count FROM (SELECT * FROM candidate_zoom_entries UNION ALL SELECT candidate_id,clicked_at,interview_at,slot_id FROM candidate_zoom_session_entries) e WHERE interview_at=${c.entry_interview_at} AND slot_id=${c.entry_slot} AND clicked_at BETWEEN interview_at-(${PRIMARY_ENTRY_WINDOW_MINUTES} * INTERVAL '1 minute') AND interview_at+INTERVAL '60 minutes'`).rows[0]?.count||0;
