...
export async function handlePrimaryEntry(callback){
  ...
  if(!allowed||!active){await telegramApi('answerCallbackQuery',{callback_query_id:callback.id,text:!allowed?'Доступна только ваша действующая запись на первое собеседование.':'Время входа в это собеседование уже завершено. Выберите новую запись, если не успели подключиться.',show_alert:true});return true;}
