...
export default async function handler(req, res) {
  ...
  const test = (await sql`SELECT sent_at,started_at,submitted_at,updated_at FROM candidate_tests WHERE candidate_id=${candidate.id} ORDER BY created_at DESC LIMIT 1`).rows[0] || null;
