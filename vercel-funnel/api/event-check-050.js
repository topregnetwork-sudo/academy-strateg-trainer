...
export default async function handler(req,res){
 ...
 const deadlines=(await sql`SELECT step,state,issued_at,due_at,error FROM stage_deadlines043 WHERE candidate_id=${candidate.id} ORDER BY issued_at`).rows.catch(()=>[]);
