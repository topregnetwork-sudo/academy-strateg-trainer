...
export default async function handler(req, res) {
  ...
  const rows = (await sql`
    SELECT DISTINCT c.id AS candidate_id,c.username
    FROM candidates c
    ...
    WHERE d.candidate_id IS NULL OR doc.candidate_id IS NULL OR sheet.candidate_id IS NULL
    ORDER BY c.id LIMIT 3
  `).rows;
  const results=[];
  for(const row of rows){
    try{const synced=await syncDriveCandidate(row.candidate_id);results.push({candidateId:row.candidate_id,username:row.username,ok:!synced.pending,folderUrl:synced.folder?.url||null});}
    catch(error){results.push({candidateId:row.candidate_id,username:row.username,ok:false,error:String(error?.message||error)});}
  }
