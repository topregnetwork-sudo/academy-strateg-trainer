...
import { previewOfflineTestingMinsk20260914, sendOfflineTestingMinsk20260914, sendOfflineTestingMinsk20260914Preview } from '../lib/offline-testing-minsk-20260914.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='PATCH'){
      const accepted=['test_1_incomplete_removed','new','experienced_not_target','interview_booked','interviewed','questionnaire','test_1_completed','test_1_passed','productivity_invited','productivity_booked','productivity_passed','productivity_failed','talent_pool','reserve_no_response','offline_testing','finalist','selection_closed','academy_contact','training','internship','hired','rejected','cancelled','collaboration','inactive'];
    ...
    if(req.method==='POST'){
      if(v.action==='preview_offline_testing_minsk_20260914') return json(res,200,{ok:true,...await previewOfflineTestingMinsk20260914()});
      if(v.action==='send_offline_testing_minsk_20260914_preview') return json(res,200,{ok:true,...await sendOfflineTestingMinsk20260914Preview()});
      if(v.action==='send_offline_testing_minsk_20260914') return json(res,200,{ok:true,...await sendOfflineTestingMinsk20260914()});
