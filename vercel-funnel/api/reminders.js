...
const questionnaireTwoCompleted = '✅ <b>Анкета 2 получена</b>\n\nСпасибо! Ответы сохранены в вашей карточке кандидата. Следующий шаг — тест из 200 вопросов. Откройте персональную ссылку кнопкой ниже.';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const questionnaireRecipients=(await sql`SELECT c.id,c.chat_id,q.id AS questionnaire_id,q.token,q.sent_at FROM candidates c LEFT JOIN candidate_questionnaire_two q ON q.candidate_id=c.id WHERE c.status IN ('interviewed','questionnaire') AND c.consent=true AND q.submitted_at IS NULL AND NOT EXISTS(SELECT 1 FROM messages m WHERE m.candidate_id=c.id AND m.kind='questionnaire_2_link' AND m.delivery_status='delivered') ORDER BY c.id LIMIT 100`).rows;
    let questionnaireSent=0,questionnaireFailed=0;
    for(const candidate of questionnaireRecipients){
      try{let qid=candidate.questionnaire_id,token=candidate.token;if(!qid){token=crypto.randomUUID().replaceAll('-','')+crypto.randomUUID().replaceAll('-','');const created=(await sql`INSERT INTO candidate_questionnaire_two(candidate_id,token) VALUES(${candidate.id},${token}) ON CONFLICT(candidate_id) DO UPDATE SET updated_at=NOW() RETURNING id,token`).rows[0];qid=created.id;token=created.token}const url=`https://topregnetwork-sudo.github.io/academy-strateg-trainer/questionnaire-2.html?token=${token}`;const linkText=`📋 <b>Ваша персональная Анкета 2:</b>\n${url}`;const messageId=await telegram(candidate.chat_id,linkText);await sql`UPDATE candidate_questionnaire_two SET status='sent',sent_at=COALESCE(sent_at,NOW()),updated_at=NOW() WHERE id=${qid}`;await sql`UPDATE candidates SET status='questionnaire',updated_at=NOW() WHERE id=${candidate.id}`;await sql`INSERT INTO messages(candidate_id,direction,kind,text,delivery_status,telegram_message_id) VALUES(${candidate.id},'out','questionnaire_2_link',${linkText},'delivered',${String(messageId||'')})`;questionnaireSent++}catch(error){questionnaireFailed++;console.error('[reminders] questionnaire link failed',{candidateId:candidate.id,message:String(error)})}
    }
    ...
    for(const item of questionnaireCompletedRows){try{const version='executive-effectiveness-2020-ru-v1';let test=(await sql`SELECT * FROM candidate_tests WHERE candidate_id=${item.candidate_id} AND questionnaire_version=${version} LIMIT 1`).rows[0];if(!test){const token=crypto.randomUUID().replaceAll('-','')+crypto.randomUUID().replaceAll('-','');test=(await sql`INSERT INTO candidate_tests(candidate_id,token,questionnaire_version,status) VALUES(${item.candidate_id},${token},${version},'pending') RETURNING *`).rows[0]}const testUrl=`https://topregnetwork-sudo.github.io/academy-strateg-trainer/test.html?token=${test.token}`;const messageId=await telegram(item.chat_id,questionnaireTwoCompleted,{reply_markup:{inline_keyboard:[[{text:'Пройти тест — 200 вопросов',url:testUrl}]]}});await sql`UPDATE candidate_questionnaire_two SET completion_notice_sent_at=NOW(),updated_at=NOW() WHERE id=${item.id}`;await sql`UPDATE candidate_tests SET status='sent',sent_at=COALESCE(sent_at,NOW()),updated_at=NOW() WHERE id=${test.id}`;await sql`INSERT INTO messages(candidate_id,direction,kind,text,delivery_status,telegram_message_id) VALUES(${item.candidate_id},'out','questionnaire_2_completed_and_test_invite',${questionnaireTwoCompleted},'delivered',${String(messageId||'')})`;try{await syncDriveCandidate(item.candidate_id)}catch(error){console.error('[reminders] drive sync after questionnaire failed',{candidateId:item.candidate_id,message:String(error)})}questionnaireCompletionSent++}catch(error){questionnaireCompletionFailed++;console.error('[reminders] questionnaire completion failed',{candidateId:item.candidate_id,message:String(error)})}}
